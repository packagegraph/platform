...
fn test_emit_package_triples_basic() -> std::io::Result<()> {
    ...
    let triple_count =
        collector.emit_package_triples(&mut writer, &pkg_data, "trixie", "stable", "amd64")?;
    ...
    assert!(
        pkg_type_count >= 2,
        "Should have dual typing (pkg:BinaryPackage + deb:BinaryPackage)"
    );
    ...
    assert!(
        lines
            .iter()
            .any(|l| l.contains("packageName") && l.contains("testpkg")),
    ...
    assert!(
        lines
            .iter()
            .any(|l| l.contains("versionString") && l.contains("1.0-1")),
        "Should have version triple"
    );

    // Check for maintainer
    assert!(
        lines.iter().any(|l| l.contains("mailto:john@example.com")),
...
fn test_emit_package_with_dependencies() -> std::io::Result<()> {
    ...
    pkg_data.insert(
        "Depends".to_string(),
        "libc6 (>= 2.36), libstdc++6 (>= 12)".to_string(),
    );
...
fn test_emit_package_with_source_field() -> std::io::Result<()> {
    ...
    assert!(
        lines
            .iter()
            .any(|l| l.contains("src/debian/trixie/testpkg/")),
...
fn test_emit_package_with_provides() -> std::io::Result<()> {
    ...
    pkg_data.insert(
        "Provides".to_string(),
        "virtual-foo, virtual-bar (= 2.0), virtual-baz".to_string(),
    );
    ...
    assert!(
        lines.iter().any(|l| l.contains("directlyProvides")),
        "Should have directlyProvides triple"
    );

    assert!(
        lines.iter().any(|l| l.contains("debProvides")),
        "Should have debProvides triple"
    );

    // Check for provided package identities
    assert!(
        lines.iter().any(|l| l.contains("virtual-foo")),
        "Should reference virtual-foo"
    );
    assert!(
        lines.iter().any(|l| l.contains("virtual-bar")),
        "Should reference virtual-bar"
    );
    assert!(
        lines.iter().any(|l| l.contains("virtual-baz")),
        "Should reference virtual-baz"
    );

    assert!(
        lines.iter().any(|l| l.contains("PackageIdentity")),
    ...
    assert!(
        !lines
            .iter()
            .any(|l| l.contains("packageName") && l.contains("(=")),
