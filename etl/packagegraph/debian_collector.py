...
class DebianCollector(BaseCollector):
        ...
        with profiler.step("Process Packages (Parallel)"):
            ...
            if not self.parallel or len(packages_data) < self.chunk_size:
                ...
                processed_count = self._process_packages_single_threaded(
                    packages_data, codename, suite, origin
                )
            else:
                ...
                for field in dep_fields:
                    if field in pkg_data:
                        ...
                        for dep_name, version_constraint in dependencies:
                            ...
                            self.g.add(
                                (dep_bnode, RDF.type, URIRef(f"{DEB}Dependency"))
                            )
                            self.g.add(
                                (
                                    dep_bnode,
                                    URIRef(f"{DEB}onPackage"),
                                    URIRef(f"{DEB}package/{quote(dep_name)}"),
                                )
                            )
                            if version_constraint:
                                self.g.add(
                                    (
                                        dep_bnode,
                                        URIRef(f"{DEB}versionConstraint"),
                                        Literal(version_constraint),
                                    )
                                )
        return pkg_map
                ...
                for line in contents_content.splitlines():
                    ...
                    for pkg_name in pkg_names_str.split(","):
                        ...
                        if clean_pkg_name in pkg_map:
                            self.g.add(
                                (
                                    pkg_map[clean_pkg_name],
                                    URIRef(
                                        "http://packagegraph.github.io/ontology/debian#fileName"
                                    ),
                                    Literal(file_path),
                                )
                            )
            ...
            for dep_key in [
                ...
                if dep_key in pkg_data:
                    ...
                    for dep_name, version_constraint in dependencies:
                        dep_bnode = BNode()
                        self.g.add(
                            (pkg_uri, URIRef(f"{DEB}{dep_key.lower()}"), dep_bnode)
                        )
                        self.g.add(
                            (dep_bnode, URIRef(f"{DEB}packageName"), Literal(dep_name))
                        )
                        if version_constraint:
                            self.g.add(
                                (
    ...
    def _process_package_chunk_wrapper(data_chunk, chunk_id):
        ...
        return DebianCollector._process_package_chunk(
            packages_data, chunk_id, codename, suite, origin
        )
    ...
    def _process_package_chunk(packages_chunk, chunk_id, codename, suite, origin):
        ...
        for pkg_data in packages_chunk:
            ...
            for key, value in pkg_data.items():
                ...
                if prop_name not in ["package", "version"]:
                    chunk_graph.add(
                        (pkg_uri, URIRef(f"{DEB}{prop_name}"), Literal(value))
                    )
    ...
    def _process_contents_chunk(lines_chunk, chunk_id, pkg_map):
        ...
        # Save to temp file
        temp_file = tempfile.NamedTemporaryFile(
            mode="w", suffix=f"_contents_chunk_{chunk_id}.ttl", delete=False
        )
        chunk_graph.serialize(destination=temp_file.name, format="turtle")
        temp_file.close()
        return temp_file.name
