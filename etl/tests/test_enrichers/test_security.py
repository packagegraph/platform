...
class TestSecurityEnricher:
    def test_enrich_writes_ntriples_for_vulnerable_package(self, tmp_path):
        ...
        mock_client.query_package_names_and_versions.return_value = [("openssl", "3.0.2-1")]
        ...
        with patch("packagegraph.enrichers.security.requests.post", return_value=osv_resp):
            enricher = SecurityEnricher(mock_client, str(output), cache_dir=str(tmp_path / "cache"))
            enricher.enrich()
