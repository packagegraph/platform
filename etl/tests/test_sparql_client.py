...
class TestSparqlQueryClient:
    def test_query_returns_bindings(self):
        ...
        data = {"results": {"bindings": [{"name": {"type": "literal", "value": "bash"}}]}}
        with patch("packagegraph.sparql_client.requests.post", return_value=_mock_response(data)):
    ...
    def test_query_github_homepages_returns_tuples(self):
        client = SparqlQueryClient("http://fuseki:3030/packagegraph")
        data = {"results": {"bindings": [
            {"pkg": {"type": "uri", "value": "https://example.org/pkg1"}, "homepage": {"type": "literal", "value": "https://github.com/owner/repo"}},
        ]}}
        with patch("packagegraph.sparql_client.requests.post", return_value=_mock_response(data)):
            results = client.query_github_homepages()
            assert results == [("https://example.org/pkg1", "https://github.com/owner/repo")]
