...
STATE_FILE="${2:-/var/lib/packagegraph/fail2ban-data/.trusted-source-last-sync}"
...
if ! podman exec fail2ban fail2ban-client ping >/dev/null 2>&1; then
...
for old_ip in "${old_ips[@]+"${old_ips[@]}"}"; do
  if ! printf '%s\n' "${ips[@]+"${ips[@]}"}" | grep -qx "$old_ip"; then
    for jail in $JAILS; do
      podman exec fail2ban fail2ban-client set "$jail" delignoreip "$old_ip" >/dev/null 2>&1 || true
    done
...
for ip in "${ips[@]+"${ips[@]}"}"; do
  if ! printf '%s\n' "${old_ips[@]+"${old_ips[@]}"}" | grep -qx "$ip"; then
    for jail in $JAILS; do
      podman exec fail2ban fail2ban-client set "$jail" addignoreip "$ip" >/dev/null 2>&1 || true
    done
