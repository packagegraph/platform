#!/bin/sh
# Collector: ubuntu-noble-riscv64
# Ported from deploy/overlays/dev/jobs/collect-ubuntu-noble-riscv64.yaml.
set -eu

# Private per-invocation scratch. /tmp is one volume shared by every
# concurrently running collector -- see README.md, "Run directories".
find /tmp/ -maxdepth 1 -type d -name 'run-*.*' -mmin +2880 -exec rm -rf {} + 2>/dev/null || true
RUN_DIR=$(mktemp -d /tmp/run-ubuntu-noble-riscv64.XXXXXXXX)
trap 'rm -rf "$RUN_DIR"' EXIT

GRAPH_URI="https://packagegraph.github.io/graph/ubuntu/noble/riscv64"

pg-collect debian --repo "http://ports.ubuntu.com/ubuntu-ports" --distro ubuntu --dist noble --component main --arch binary-riscv64 -o "$RUN_DIR/packages.nt"
/app/scripts/upload-nt.sh "$RUN_DIR/packages.nt" "$GRAPH_URI" "http://ports.ubuntu.com/ubuntu-ports"
