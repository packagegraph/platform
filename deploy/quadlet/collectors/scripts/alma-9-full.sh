#!/bin/sh
# Collector: alma-9-full
# New 2026-09-10 -- no Kubernetes equivalent. Fills in the "almalinux/9"
...
RUN_DIR=$(mktemp -d /tmp/run-alma-9-full.XXXXXXXX)
trap 'rm -rf "$RUN_DIR"' EXIT

GRAPH_URI="https://packagegraph.github.io/graph/almalinux/9"

CACHE_DIR=/tmp/cache/alma-9-full
MINIO_CACHE="pgraph/${MINIO_BUCKET}/collector-cache/alma-9-full"
...
pg-collect rpm-full \
  ...
  --url https://repo.almalinux.org/almalinux/9/BaseOS/aarch64/os/ \
  --distro almalinux --release 9 \
  --with-spec --with-maintainers \
  --cache-dir "${CACHE_DIR}" \
  -o "$RUN_DIR/alma-9.nt"

/app/scripts/upload-nt.sh "$RUN_DIR/alma-9.nt" "$GRAPH_URI"
